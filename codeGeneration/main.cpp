...
//stores the variables in the code
void buildSymbolTable(TreeNode* node, SymbolTable* symbolTable)
{
    //base case
    if(node->node_kind == ID_NODE || node->node_kind == READ_NODE || node->node_kind == ASSIGN_NODE)
    {
    ...
    for(int i = 0; i < MAX_CHILDREN; i++)
    {
        if(node->child[i])
        {
             buildSymbolTable(node->child[i], symbolTable);
        }
    }

    if(node->sibling)
    {
         buildSymbolTable(node->sibling, symbolTable);
    }
...
int run(TreeNode* node, SymbolTable* symbolTable, int* memory)
{
    //base case 1
    ...
    if(node->node_kind == ID_NODE)
    {
        ...
        int var = memory[varInfo->memloc];
        return var;
    }

    int leftChild, rightChild;
    leftChild = run(node->child[0], symbolTable, memory);
    rightChild = run(node->child[1], symbolTable, memory);
...
void runCode(TreeNode* node, SymbolTable* symbolTable, int* memory)
{
    if(node->node_kind == IF_NODE)
    {
        ...
        if(condition)
        {
            //run the body of the if-statement
    ...
    {
        int output = run(node->child[0], symbolTable, memory);
...
void codeGeneration(TreeNode* parseTree, SymbolTable* symbolTable)
{
    int numOfVars = symbolTable->num_vars;
    int* memory = new int[numOfVars];
    ...
    runCode(parseTree, symbolTable, memory);
    delete[] memory;
}
...
int main()
{
    ...
    //generating the symbol table (that has the initial values of the variables)
